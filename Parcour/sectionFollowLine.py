...
class SectionFollowLine(Section):        # done by Jan
    ...
    def __init__(self):
        ...
        self.DRIVE_SPEED = 120                   # mm/s     -> if changed here, also adjust in "if self.k == 500"
    ...
    def run_one_step(self, robot):
        ...
        if seeing_line:
            ...
            if self.state == State.SEEING_LINE:          # line should only be lost if there is a sharp bend in the line to the left (max. 90°) or a gap
                # robot.drive(0, -self.TURN_SPEED)       # searching line on left side -> this would be the logical way, but we see the left edge of the line and think we wrongfully found the line again
                robot.drive(20, -self.TURN_SPEED)        #  -> driving forward with 20 mm/s the first 30 degrees to avoid this problem
                robot.reset_distance_and_angle()         # reset angle to turn 100 degrees max from this point (100 degrees = gap)
                ...
                if angle_turned_since_line_lost < -30 and angle_turned_since_line_lost > -33:   # after 30 degrees stop driving forward, since left edge of the line is not a problem anymore
    ...
    def pd_regler(self, robot, reflection):
        ...
        speed = self.DRIVE_SPEED - (self.DRIVE_SPEED / 30) * abs(error)     # turning in place when error > 30
        speed = max(speed, 0)
        robot.drive(drive_speed=speed, turn_rate=correction)

    def drive_around_gap(self, robot):
        robot.drive(drive_speed=(self.TURN_SPEED / 5), turn_rate=self.TURN_SPEED)   # turn back straight, since gap was detected after 100 degrees turn
    ...
    def drive_around_obstacle(self, robot):
        ...
        while robot.driven_distance() < already_distance_driven + 470:
            pass
        robot.reset_distance_and_angle()
        robot.drive(drive_speed=(self.DRIVE_SPEED / 3), turn_rate=0)
        self.DRIVE_SPEED = 30                                        # because of overshoot when finding the line again
        self.k = 0                                                   # reset Drivespeed after 500 Iterations (about 2-3seconds) 
